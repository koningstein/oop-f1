"""
Screen 4: Realtime Data Car 2
All functions for displaying realtime telemetry data for Car 2
(Identical to Screen 3, but for the second simulator/car)
...
Note: This is identical to screen3.py but can be used for a second car/simulator
"""
...
def dashboard_live_telemetrie():
    ...
    """
    print("\n🏎️  DASHBOARD - LIVE TELEMETRIE AUTO 2")
    print("=" * 80)
    print("Druk op ENTER om te stoppen\n")
...
def damage_monitoring():
    ...
    def handle_damage(packet: CarDamagePacket):
        ...
        print(f"\r\n", end='')
        print(f"\r\n🏁 TYRES:", end='')
        print(f"\r\n   RL: {player.tyres_damage[0]:3.0f}%  [{_damage_bar(player.tyres_damage[0])}]", end='')
        print(f"\r\n   RR: {player.tyres_damage[1]:3.0f}%  [{_damage_bar(player.tyres_damage[1])}]", end='')
        print(f"\r\n   FL: {player.tyres_damage[2]:3.0f}%  [{_damage_bar(player.tyres_damage[2])}]", end='')
        print(f"\r\n   FR: {player.tyres_damage[3]:3.0f}%  [{_damage_bar(player.tyres_damage[3])}]", end='')
        print(f"\r\n", end='')
        print(f"\r\n🛫 WINGS:", end='')
        print(f"\r\n   Front L: {player.front_left_wing_damage:3.0f}%  [{_damage_bar(player.front_left_wing_damage)}]", end='')
        print(f"\r\n   Front R: {player.front_right_wing_damage:3.0f}%  [{_damage_bar(player.front_right_wing_damage)}]", end='')
        print(f"\r\n   Rear:    {player.rear_wing_damage:3.0f}%  [{_damage_bar(player.rear_wing_damage)}]", end='')
        print(f"\r\n", end='')
        print(f"\r\n🔧 COMPONENTS:", end='')
        print(f"\r\n   Engine:  {player.engine_damage:3.0f}%  [{_damage_bar(player.engine_damage)}]", end='')
        print(f"\r\n   Gearbox: {player.gearbox_damage:3.0f}%  [{_damage_bar(player.gearbox_damage)}]", end='')
        print(f"\r\n   Floor:   {player.floor_damage:3.0f}%  [{_damage_bar(player.floor_damage)}]", end='')
        print(f"\r\n", end='')
...
def vergelijk_met_auto1():
    """
    Vergelijk Auto 2 met Auto 1: Side-by-side comparison
    """
    print("\n🔄 VERGELIJKING AUTO 1 vs AUTO 2")
    print("=" * 80)
    print("Druk op ENTER om te stoppen\n")
    
    print("⚠️  Opmerking: Deze functie vergelijkt de player car met zichzelf")
    print("    In een echte multi-car setup zou dit 2 verschillende cars vergelijken\n")
    
    def handle_telemetry(packet: CarTelemetryPacket):
        player = packet.get_player_telemetry()
        
        print(f"\r" + "=" * 80, end='')
        print(f"\r\n{'Metric':<20} {'Auto 1':<25} {'Auto 2':<25}", end='')
        print(f"\r\n" + "-" * 80, end='')
        print(f"\r\nSpeed", end='')
        print(f"\r   {player.speed:3d} km/h                   {player.speed:3d} km/h", end='')
        print(f"\r\nGear", end='')
        print(f"\r   {player.gear:2d}                          {player.gear:2d}", end='')
        print(f"\r\nRPM", end='')
        print(f"\r   {player.engine_rpm:5d}                      {player.engine_rpm:5d}", end='')
        print(f"\r\nThrottle", end='')
        print(f"\r   {player.throttle:5.1%}                    {player.throttle:5.1%}", end='')
        print(f"\r\nBrake", end='')
        print(f"\r   {player.brake:5.1%}                    {player.brake:5.1%}", end='')
        print(f"\r\n" + "=" * 80, end='', flush=True)
