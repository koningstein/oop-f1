...
Toont de menu's, statussen en foutmeldingen.
(GECORRIGEERDE VERSIE 4: Merge van originele file en de fix)
"""
...
from services import UDPListener # Zorg ervoor dat deze import er is
...
class MenuView:
    ...
    def show_welcome(self):
        ...
        print("  - UDP Port: 20777 (of configureer in config.py)")
        print()
        print("=" * 80)
        print()
    ...
    def display_main_menu(self):
        """
        Toon het hoofdmenu (1-6)
        """
        print("\n[ HOOFDMENU ]")
        print("  1. Dashboard Overzicht")
        print("  2. Timing & Sectoren")
        print("  3. Live Telemetry")
        print("  4. Klassement")
        print("  5. Vergelijking")
        print("  6. Sessie Geschiedenis")
        print("  0. Afsluiten")
        print("-" * 80)

    # --- NIEUWE METHODE ---
    def display_submenu(self, screen_number: int):
        """
        Toon het submenu voor een specifiek scherm
        """
        options = self.menu_controller.get_submenu_options(screen_number)

        print(f"\n[ SCHERM {screen_number} - OPTIES ]")
        if not options:
            print("  Geen submenu opties beschikbaar.")
    ...
    def show_menu(self):
        """
        Toon het huidige menu (hoofdmenu of submenu)
        """
        if self.menu_controller.get_current_submenu() is not None:
            # We zitten IN een functie (bv. 1.5).
            # De main loop toont de prompt
            return

        if self.menu_controller.is_in_submenu_mode():
            # We zitten IN een submenu (bv. Scherm 1).
            current_screen = self.menu_controller.get_current_screen()
            self.display_submenu(current_screen)  # CORRECTE NAAM
        else:
            # We zitten in het hoofdmenu.
            self.display_main_menu()  # CORRECTE NAAM
    ...
    def get_user_input(self) -> str:
        ...
        """
        if self.menu_controller.is_in_submenu_mode():
            prompt = f"Kies optie voor Scherm {self.menu_controller.get_current_screen()} (B=terug): "
        elif self.menu_controller.get_current_submenu() is not None:
            # Deze prompt wordt in de praktijk niet gebruikt door de
            # non-blocking 'run' loop, maar we laten hem staan.
            prompt = "Actie (B=terug, 0=quit): "
        else:
            prompt = "Kies een optie (1-6, 0=quit): "

        return input(prompt).strip()

    def show_status(self, udp_listener: UDPListener):
        """
        ...
        if self.menu_controller.is_in_submenu_mode():
            print(f"  Status: Submenu selectie")
        elif current_submenu is not None:
            # Probeer de naam op te halen, maar vang KeyErrors op
            try:
                name, _ = self.menu_controller.screen_submenu_options[current_screen][current_submenu]
                print(f"  Actieve functie: {current_screen}.{current_submenu} - {name}")
            except KeyError:
                 print(f"  Actieve functie: {current_screen}.{current_submenu}")
        else:
            print(f"  Status: Hoofdmenu")
        print("="*80) # Deze was je vergeten in je 'oude' file, maar hoort hier wel
    ...
    def show_submenu_details(self, screen_number: int):
        ...
        """
        if screen_number not in self.menu_controller.screen_submenu_options:
        ...
        options = self.menu_controller.screen_submenu_options[screen_number]
