...
class DataValidator:
    ...
    # --- NIEUWE FUNCTIE (DE VALIDATIE LES) ---
    def parse_lap_validation_flags(self, flags: int) -> Dict[str, bool]:
        """
        Interpreteert de m_lapValidBitFlags van Packet 11 (Session History).

        Specificatie (F1 25 Telemetry Output Structures.txt):
        m_lapValidBitFlags;
        // 0x01 bit set-lap valid,      0x02 bit set-sector 1 valid
        // 0x04 bit set-sector 2 valid, 0x08 bit set-sector 3 valid

        Args:
            flags: De 8-bit integer (uint8) van het pakket.

        Returns:
            Een dictionary met de boolean validatie status.
        """

        # We gebruiken bitwise AND operatoren om de flags te controleren
        validation_data = {
            'is_valid': (flags & 0x01) != 0,
            'sector1_valid': (flags & 0x02) != 0,
            'sector2_valid': (flags & 0x04) != 0,
            'sector3_valid': (flags & 0x08) != 0
        }

        self.logger.debug(f"Parsing flags {flags}: {validation_data}")
